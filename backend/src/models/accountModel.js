...
Account.findAccountsByUserId = (id, result) => {
  sql.query(
    "SELECT * from account a inner join account_plan p on a.plan_ID = p.plan_ID where customer_ID = ?",
    id,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
...
Account.findSavingsAccountsByUserId = (id, result) => {
  sql.query(
    "SELECT * from account a inner join account_plan p on a.plan_ID = p.plan_ID where type='Savings' and customer_ID = ?",
    id,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
...
Account.updateInterests = (result) => {
  sql.query(
    "",
    (err, res) => {
      if (err) {
