...
NormalApplication.create = (newNormalApplication, result) => {
  sql.query("INSERT INTO normal_application SET ?", newNormalApplication, (err, res) => {
    if (err) {
    ...
    console.log("created normal application: ", { id: res.insertId, ...newNormalApplication });
    result(null, { id: res.insertId, ...newNormalApplication });
  });
};

NormalApplication.findById = (id, result) => {
  sql.query("SELECT * FROM normal_application WHERE app_ID = ?", [id], (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
NormalApplication.updateById = (id, normalApplication, result) => {
  sql.query(
    ...
    [normalApplication.branch_ID, normalApplication.acc_ID,normalApplication.amount,normalApplication.is_approved, normalApplication.app_date, normalApplication.loan_ID, id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
      ...
      console.log("updated normal application: ", { id: id, ...normalApplication });
...
NormalApplication.remove = (id, result) => {
  sql.query("DELETE FROM normal_application WHERE app_ID = ?", id, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
