...
const OnlineApplication = function (onlineApplication) {
  ...
  this.loan_plan = onlineApplication.loan_plan;
...
OnlineApplication.create = (newOnlineApplication, result) => {
  console.log({ newOnlineApplication });
  sql.query(
    "call Creating_Online_application(?, ?, ?, ?);",
    [
      newOnlineApplication.fd_ID,
      newOnlineApplication.acc_ID,
      newOnlineApplication.amount,
      newOnlineApplication.loan_plan,
    ],
    (err, res) => {
      if (err) {
      ...
      console.log("created online application: ", {
        id: res.insertId,
...
OnlineApplication.findById = (id, result) => {
  sql.query(
    "SELECT * FROM online_application WHERE app_ID = ?",
    [id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
...
OnlineApplication.updateById = (id, onlineApplication, result) => {
  sql.query(
    ...
    [
      onlineApplication.fd_ID,
      onlineApplication.acc_ID,
      onlineApplication.amount,
      onlineApplication.app_date,
      onlineApplication.loan_ID,
      id,
    ],
    (err, res) => {
      if (err) {
      ...
      console.log("updated online application: ", {
        id: id,
        ...onlineApplication,
      });
...
OnlineApplication.remove = (id, result) => {
  sql.query(
    "DELETE FROM online_application WHERE app_ID = ?",
    id,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
