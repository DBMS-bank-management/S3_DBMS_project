...
exports.create = (req, res) => {
  ...
  TransactionModel.create(transaction, (err, data) => {
    if (err)
      res.status(500).send({
        message:
          err.message || "Some error occurred while creating the TransactionModel.",
      });
    else res.send(data);
  });
};
  ...
  TransactionModel.updateById(req.params.id, new TransactionModel(req.body), (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        ...
        res.status(500).send({
          message: "Error updating transaction with trans_ID " + req.params.id,
        });
      }
    } else res.send(data);
  });
  ...
  TransactionModel.remove(req.params.id, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        ...
        res.status(500).send({
          message: "Could not delete transaction with trans_ID " + req.params.id,
        });
      }
